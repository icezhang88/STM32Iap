...
void fmc_erase_pages(void)
{
  ...
  NbOfSectors = GetSector(ADDR_FLASH_SECTOR_2) - FirstSector + 1;
...
uint32_t FLASH_If_Write(__IO uint32_t *FlashAddress, uint32_t Data, uint16_t DataLength)
{
  ...
  for (i = 0; (i < DataLength) && (*FlashAddress <= (USER_FLASH_END_ADDRESS - 4)); i++)
  {
    ...
    ptr = (uint32_t *)Data;
    data0 = *(ptr + i);
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, address, data0) == HAL_OK)
    {
      *FlashAddress += 4;
    }
    else
...
uint32_t SerialKeyPressed(uint8_t *key)
{
  ...
  {
    HAL_UART_Receive(&huart1, key, 1, 100);
    return 1;
  }
  return 0;
}
    ...
    for (packets_received = 0, file_done = 0, buf_ptr = buf;;)
    {
      ...
      default:
        ...
        if (TimeOutCounter >= 50)
        {
          ...
          RCC_DeInit();
          NVIC_DeInit();
          JumpAddress = *(__IO uint32_t *)(APPLICATION_ADDRESS + 4); // ������ת��ַ��0x08004000
          JumpToApplication = (pFunction)JumpAddress;
          /* Initialize user application's Stack Pointer */
          __set_MSP(*(__IO uint32_t *)APPLICATION_ADDRESS);
          JumpToApplication(); // ��ת��Ӧ�ó���
        }
...
void Main_Menu(void)
{
  ...
  /**
  uint32_t flag = Read_IAP_Flag();
	 
  char addressValue[30]={0};
  sprintf(addressValue,"value:%d \r\n",flag);
  SerialPutString(addressValue);
	 
	 
  if (flag == USE_IAP_FLAG_ENABLED)
  {
    SerialPutString("\r\n  STM32F407 In-Application Programming Application  \r\n");
    SerialDownload();
  }
